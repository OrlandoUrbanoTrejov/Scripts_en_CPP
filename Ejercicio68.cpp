...
int main(){
   ...
   if(Opcion >= 0 && Opcion <= 6){
       for(int j = 0; j < 9; j++){
	       std::cout << j << ") " << Menu[Opcion][j] << std::endl;
       }
   }
   std::cout << "Elige la unidad de entrada: ";
   std::cin >> UnidadEntrada;
   std::cout << "Elige la unidad de destino ";
   std::cin >> UnidadDestino;
   std::cout << "Introduce el valor a convertir: ";
   std::cin >> Valor;

   Resultado = Valor * (Operaciones[Opcion][UnidadDestino] / Operaciones[Opcion][UnidadEntrada]);
   std::cout << Valor << " " << Menu[Opcion][UnidadEntrada] << " = " << Resultado << " " << Menu[Opcion][UnidadDestino] << std::endl;  
return 0;
}
